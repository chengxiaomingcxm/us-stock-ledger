...
| 原有核心功能正常 | ✅ | 148 单元 + 362 原生断言 + 26 端到端用例全绿（run `35449140805` / `35449140797`）——362 是**当时**的数字；V1.0 收尾轮后固定断言为 413，2026-09-20 起（合成结单 fixture）为 429，见 `docs/ENGLISH_UI_FINAL_AUDIT.md` §3 |
...
对应到本轮的可见产出：真实业务逻辑（成本/收益/现金口径与不变式）、数据处理（CSV 与 PDF 导入 + 校验 + 预览）、API 集成（三家行情源 + 归一化 + 降级）、UI（SwiftUI 四页 + 图表 + 日历 + 无障碍）、测试（四层，当时为 362/148/26，现为 429/148/26）、发布流程（双 workflow + 校验式发布脚本 + 带校验和的未签名 IPA）。
