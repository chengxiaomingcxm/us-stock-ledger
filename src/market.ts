...
export function parseDailyClose(raw: unknown, symbol: string, now = Date.now()): Quote {
...
export async function fetchClose(symbol: string): Promise<Quote> {
  const url = `https://query2.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(providerSymbol(symbol))}?interval=1d&range=1mo&includePrePost=false`;
  let body: unknown;
  if (Capacitor.isNativePlatform()) {
    const response = await CapacitorHttp.get({ url, responseType: 'json', connectTimeout: 12_000, readTimeout: 12_000, headers: { 'User-Agent': 'StockLedger/1.1 (personal portfolio)' } });
  ...
  return parseDailyClose(body, symbol);
}
