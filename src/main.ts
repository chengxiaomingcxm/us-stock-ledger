...
function cashPanel(s:ReturnType<typeof calculate>){
 ...
 const boundaryNote=t.excludedTrades||t.excludedRecords?`期初前有 ${t.excludedTrades} 笔交易、${t.excludedRecords} 笔资金记录，已包含在期初余额中，仅保留备查，不重复计入。`:'';
 return `<section class="panel cash-panel"><div class="section-heading"><h2>现金账本</h2><span class="caption">入金出金不计盈亏</span></div>
 <div class="cash-head"><div class="cash-balance"><span>当前现金余额</span><strong data-testid="cash-balance">${t.balance===undefined?'待设置期初':money(t.balance)}</strong><small>${t.balance===undefined?'旧账本没有现金记录，先设置期初余额，之后的买卖和资金流水会自动联动现金':data.cash?.opening?`期初 ${esc(data.cash.opening.date)} 开始前 · ${esc(data.cash.opening.note||'—')}`:''}</small></div><div class="cash-assets"><span>总资产（现金 + 持仓市值）</span><strong data-testid="total-assets">${assets===undefined?'—':money(assets)}</strong></div></div>
 <dl class="cash-totals"><div><dt>期初余额</dt><dd data-testid="cash-opening">${data.cash?.opening?money(data.cash.opening.amount):'未设置'}</dd></div><div><dt>累计入金</dt><dd data-testid="cash-deposit">${money(t.deposit)}</dd></div><div><dt>累计出金</dt><dd data-testid="cash-withdraw">${money(t.withdraw.neg())}</dd></div><div><dt>分红到账（扣税）</dt><dd data-testid="cash-dividend">${money(t.dividend.minus(t.tax))}</dd></div><div><dt>预扣税费</dt><dd data-testid="cash-tax">${money(t.tax.neg())}</dd></div><div><dt>账户费用</dt><dd data-testid="cash-fee">${money(t.fee.neg())}</dd></div><div><dt>买入支出（含费）</dt><dd data-testid="cash-buy">${money(t.buyOut.neg())}</dd></div><div><dt>卖出收入（扣费）</dt><dd data-testid="cash-sell">${money(t.sellIn)}</dd></div><div><dt>买卖净现金流</dt><dd data-testid="cash-trade-net" class="${tone(t.tradeNet)}">${signedMoney(t.tradeNet)}</dd></div></dl>
 <div class="cash-nets"><span>外部资金净流入<strong data-testid="cash-external">${signedMoney(t.opening.plus(t.externalNet))}</strong></span><span>投资现金流入（分红 − 税费 − 费用）<strong data-testid="cash-invest">${signedMoney(t.investNet)}</strong></span></div>
 ${boundaryNote?`<p class="form-hint">${esc(boundaryNote)}</p>`:''}
 ...
 ${records.length?`<div class="cash-records" data-testid="cash-records"><div class="cash-record cash-record-head"><span>类型</span><span>金额与说明</span><span></span></div>${records.map(r=>{const before=isBeforeOpening(r.date,data);return `<div class="cash-record"><span class="cash-kind ${r.kind}">${kindLabel(r.kind)}${before?' · 期初前':''}</span><div class="cash-record-main"><strong class="${before?'':r.kind==='withdraw'||r.kind==='fee'?'':r.kind==='deposit'||r.kind==='dividend'?'positive':''}">${r.kind==='deposit'||r.kind==='dividend'?'+':''}${money(r.kind==='withdraw'||r.kind==='fee'?D(r.amount).neg():cashNet(r))}</strong><small>${esc(r.date)}${r.symbol?` · ${esc(r.symbol)}`:''}${r.note?` · ${esc(r.note)}`:''}${r.source==='import'?' · CSV':''}${before?' · 已含在期初余额':''}</small></div><div class="cash-record-actions"><button class="text-button" data-cash-edit="${esc(r.id)}" ${demo||blocked?'disabled':''}>编辑</button><button class="text-button delete" data-cash-delete="${esc(r.id)}" ${demo||blocked?'disabled':''}>删除</button></div></div>`;}).join('')}</div>`:`<p class="small-empty">还没有现金记录。设置期初余额后，可记录入金、出金、分红和账户费用，也可从券商 CSV 导入。</p>`}
 <p class="form-hint">余额 = 期初 + 入金 − 出金 + 分红（扣税）− 费用 + 卖出收入 − 买入支出；买卖是资产转换，不计入盈亏。期初日之后的交易才联动现金，之前的交易视为已包含在期初余额中。</p></section>`;
}
function insights(s:ReturnType<typeof calculate>){const t=cashTotals(data);return `${marketStatus()}${historyPanel(data,historyMonth)}<section class="panel profit-summary"><div class="eyebrow">INVESTMENT RETURN</div><h2>累计投资收益</h2><div class="summary-number ${tone(s.totalProfit)}">${signedMoney(s.totalProfit)}</div><p>已实现收益 + 当前持仓浮动收益（证券口径）</p><div class="summary-split"><div><span>已实现收益</span><strong class="${tone(s.realized)}">${signedMoney(s.realized)}</strong></div><div><span>浮动收益</span><strong class="${tone(s.unrealized)}">${signedMoney(s.unrealized)}</strong></div></div><div class="summary-split summary-cash"><div><span>分红净额（扣税）</span><strong class="${tone(t.investNet.plus(t.fee))}">${signedMoney(t.dividend.minus(t.tax))}</strong></div><div><span>账户费用</span><strong class="${tone(t.fee.neg())}">${signedMoney(t.fee.neg())}</strong></div></div><div class="account-profit"><span>账户总收益（证券 + 分红净额 − 费用，全历史）</span><strong class="${s.totalProfit===undefined?'neutral':tone(s.totalProfit.plus(t.investNetAll))}" data-testid="account-profit">${s.totalProfit===undefined?'—':signedMoney(s.totalProfit.plus(t.investNetAll))}</strong></div><p class="form-hint">入金出金永远不算收益；除息日证券市值回落会与分红到账相互抵消。今日盈亏和收益日历仍只统计证券。</p>${s.missing.length?'<p class="notice-inline">部分股票缺少股价，累计收益暂未计算。</p>':''}</section>${cashPanel(s)}<div class="two-columns"><section class="panel"><div class="section-heading"><h2>持仓成本分布</h2><span class="caption">按成本占比</span></div>${!s.open.length?'<p class="small-empty">暂无持仓</p>':s.open.map((p,i)=>{const pc=p.cost.div(s.cost).mul(100).toFixed(1);return `<div class="allocation"><div><b>${esc(p.symbol)}</b><span>${pc}% <small>${money(p.cost)}</small></span></div><div class="bar"><div class="color-${i%4}" style="width:${pc}%"></div></div></div>`;}).join('')}</section><section class="panel"><div class="section-heading"><h2>资金记录</h2></div><dl class="totals"><div><dt>累计买入支出</dt><dd>${money(s.buyTotal)}</dd></div><div><dt>累计卖出收入</dt><dd>${money(s.sellTotal)}</dd></div><div><dt>累计手续费</dt><dd>${money(s.fees)}</dd></div><div><dt>交易笔数</dt><dd>${data.trades.length} 笔</dd></div></dl></section></div><section class="panel"><div class="section-heading"><h2>各股票已实现收益</h2></div>${!s.positions.length?'<p class="small-empty">卖出后可在这里查看收益</p>':s.positions.map(p=>`<div class="realized-row"><b>${esc(p.symbol)} <small>${p.quantity.eq(0)?'已平仓':'持仓中'}</small></b><strong class="${tone(p.realized)}">${signedMoney(p.realized)}</strong></div>`).join('')}</section>`;}
...
function cashModal(kind: CashKind|'opening', record?: CashRecord){
 ...
 ${isOpening?`<p class="form-hint">期初余额是“期初日期当天开始前”的现金，请对照券商账单填写。该日期当天及之后的入金、出金、分红、费用和股票买卖会联动余额；之前的记录视为已包含在期初余额中，保留备查、不重复计入。允许为零，不支持负数（不含融资）。</p>`:''}
 ...
 form.onsubmit=e=>{e.preventDefault();void submit(async()=>{
  ...
  const rec:CashRecord={id:record?.id??crypto.randomUUID(),sequence:record?.sequence??(Math.max(-1,...(data.cash?.records??[]).map(x=>x.sequence))+1),date,kind:k,amount,tax:k==='dividend'?get('tax')||undefined:undefined,symbol:(k==='dividend'||k==='fee')&&sym?sym:undefined,note,source:record?.source??'manual',externalId:record?.externalId};
...
function csvImportModal(){
 ...
 let fallback:CashKind|undefined;
 let orderConfirmed=false;
 modal('导入券商 CSV',`<p class="form-hint">选择带表头的交易或资金流水 CSV（逗号 / 分号 / 制表符分隔，最多 5,000 行）。先核对字段映射与预览，确认后才写入账本；已导入的记录不可重复写入，数值相同的疑似重复默认不勾选。<button type="button" class="text-button" data-help="cash">格式说明</button></p>
 ...
 let placement:'after'|'before'='after';
 ...
 modalRoot.querySelectorAll<HTMLInputElement>('[name="csv-kind"]').forEach(r=>r.onchange=()=>{kind=r.value as 'trades'|'cash';mapping=defaultMapping();fallback=undefined;orderConfirmed=false;placement='after';if(table)renderAll();});
 ...
 const typeMapped=()=>kind==='cash'&&(mapping as Mapping<CashField>).type!==undefined;
 function renderMapping(){
  ...
  const needFallback=kind==='cash'&&!typeMapped();
  mapBox.innerHTML=`<div class="csv-mapping"><div class="section-heading"><h3>字段映射</h3><span class="caption">把 CSV 列对应到账本字段；未选择币种列时按美元处理</span></div><div class="csv-map-grid">${fieldsOf().map(f=>`<label>${labels[f]}<select data-map="${f}" aria-label="${labels[f]}列">${opts((mapping as Record<string,number|undefined>)[f])}</select></label>`).join('')}${needFallback?`<label>统一类型<select id="csv-fallback" aria-label="统一类型"><option value="" ${fallback===undefined?'selected':''}>请选择</option>${[['deposit','入金'],['withdraw','出金'],['dividend','分红'],['fee','费用']].map(([k,l])=>`<option value="${k}" ${fallback===k?'selected':''}>${l}</option>`).join('')}</select></label>`:''}</div>${needFallback?'<p class="form-hint csv-fallback-hint">没有类型列：请为整批记录选择统一类型后才能导入。</p>':''}</div>`;
  mapBox.querySelectorAll<HTMLSelectElement>('[data-map]').forEach(s=>s.onchange=()=>{mapping=readMapping();if(!typeMapped())fallback=undefined;renderMapping();renderPreview();});
  const fb=mapBox.querySelector<HTMLSelectElement>('#csv-fallback');if(fb)fb.onchange=()=>{fallback=fb.value?fb.value as CashKind:undefined;renderPreview();};
 }
 const selectedCount=()=>{if(!analysis)return 0;return analysis.rows.filter(r=>{const box=body.querySelector<HTMLInputElement>(`[data-import-row="${r.line}"]`);return r.status!=='duplicate'&&box?.checked;}).length;};
 const selectionError=():string|undefined=>{
 ...
 function updateConfirm(){
  ...
  body.querySelector('#csv-sel-error')?.remove();
  if(err){const d=document.createElement('div');d.className='csv-batch-error';d.id='csv-sel-error';d.textContent=`选中行校验未通过：${err}`;body.prepend(d);}
 }
 function renderPreview(){
  analyze();
  if(!analysis)return;
  const ok=analysis.rows.filter(r=>r.status==='new').length;
  const sus=analysis.rows.filter(r=>r.status==='suspected').length;
  const dup=analysis.rows.filter(r=>r.status==='duplicate').length;
  const rowsHtml=kind==='trades'
   ?(analysis as TradeImport).rows.map(r=>{const t=r.trade;const dupBadge=r.status==='duplicate'?' · 已导入（不可重复写入）':r.status==='suspected'?' · 与已有记录数值相同，疑似重复（同参数独立成交可勾选）':'';
    return `<label class="csv-row ${r.status==='suspected'?'csv-dup':''} ${r.status==='duplicate'?'csv-locked':''}"><input type="checkbox" data-import-row="${r.line}" ${r.status==='new'?'checked':''} ${r.status==='duplicate'?'disabled':''}><span class="csv-row-cells"><b>${esc(t.symbol)} · ${t.side==='buy'?'买入':'卖出'} ${quantity(t.quantity)} 股 × ${money(t.price)}${r.time?` · ${esc(r.time)} 美东`:''}</b><small>${esc(t.date)}${t.externalId?` · 编号 ${esc(t.externalId)}`:''}${t.fee!=='0'?` · 手续费 ${money(t.fee)}`:''}${t.note?` · ${esc(t.note)}`:''}${dupBadge}</small></span></label>`;}).join('')
   :(analysis as CashImport).rows.map(r=>{const c=r.record;const dupBadge=r.status==='duplicate'?' · 已导入（不可重复写入）':r.status==='suspected'?' · 与已有记录数值相同，疑似重复（独立流水可勾选）':'';
    return `<label class="csv-row ${r.status==='suspected'?'csv-dup':''} ${r.status==='duplicate'?'csv-locked':''}"><input type="checkbox" data-import-row="${r.line}" ${r.status==='new'?'checked':''} ${r.status==='duplicate'?'disabled':''}><span class="csv-row-cells"><b>${kindLabel(c.kind)}${c.symbol?` · ${esc(c.symbol)}`:''} · ${money(c.amount)}${c.tax?` − 税费 ${money(c.tax)}`:''}</b><small>${esc(c.date)}${c.externalId?` · 编号 ${esc(c.externalId)}`:''}${c.note?` · ${esc(c.note)}`:''}${dupBadge}</small></span></label>`;}).join('');
 ...
 fileInput.onchange=()=>{const f=fileInput.files?.[0];if(!f)return;void (async()=>{
  try{if(f.size>2_000_000)throw Error('文件过大，请选择 2 MB 以内的 CSV。');
   const {text,encoding}=decodeCsvBytes(await f.arrayBuffer());
   table=parseCsv(text);nameEl.textContent=`${f.name} · ${encoding} · ${table.rows.length} 行数据 · ${table.header.length} 列`;error.textContent='';modalGuard=()=>true;mapping=defaultMapping();fallback=undefined;orderConfirmed=false;placement='after';renderAll();
 ...
 confirmBtn.onclick=()=>{
  ...
  void submit(async()=>{
   let next:Ledger;
   if(kind==='trades'){const selected=(analysis as TradeImport).rows.filter(r=>boxes.find(b=>b.dataset.importRow===String(r.line))?.checked).map(r=>r.trade);next=applyTradeImport(data,selected,()=>crypto.randomUUID(),{insertBeforeSameDay:placement==='before'});}
   else{const selected=(analysis as CashImport).rows.filter(r=>boxes.find(b=>b.dataset.importRow===String(r.line))?.checked).map(r=>r.record);next=applyCashImport(data,selected,()=>crypto.randomUUID());}
   await commit(next,'CSV 导入前');
  });
 };
}
...
function exportBackup(compatible=false){
 if(demo||blocked)return;
 if(compatible){
  const cashN=data.cash?.records.length??0,hasOpening=!!data.cash?.opening,histN=data.history?.closes.length??0;
  modal('导出各版本通用备份？',`<p class="dialog-copy">通用备份供旧版本读取，本次导出将不包含以下数据：<br>${hasOpening?'· 期初余额<br>':''}${cashN?`· 现金记录 ${cashN} 笔<br>`:''}· 收益历史收盘价 ${histN} 条</p><p class="form-hint">两台 1.26 设备之间请使用「导出完整备份」；旧版重新导出后不会保留现金与历史数据。</p><p id="form-error" class="form-error" role="alert"></p><div class="form-actions"><button class="secondary" id="cancel-compat-export">取消</button><button class="primary" id="confirm-compat-export">继续导出</button></div>`);
  modalRoot.querySelector<HTMLButtonElement>('#cancel-compat-export')!.onclick=()=>doClose();
  modalRoot.querySelector<HTMLButtonElement>('#confirm-compat-export')!.onclick=()=>{doClose();void exportBackupCore(true);};
  return;
 }
 void exportBackupCore(false);
}
async function exportBackupCore(compatible=false){const b=app.querySelector<HTMLButtonElement>('#export')!;b.disabled=true;try{const text=backupText(data,compatible),filename=`stock-ledger-${compatible?'compatible-':''}${today()}-${Date.now()}.json`;if(Capacitor.isNativePlatform()){const result=await Filesystem.writeFile({path:filename,data:text,directory:Directory.Cache,encoding:Encoding.UTF8});await Share.share({title:'持仓账本备份',files:[result.uri],dialogTitle:'保存账本备份'});}else{const url=URL.createObjectURL(new Blob([text],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download=filename;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}const next={...appearance,lastExport:Date.now()};await saveAppearance(next);appearance=next;render();toast('备份已导出，请确认已保存到文件或其他安全位置');}catch(e){toast(`未完成导出：${e instanceof Error?e.message:String(e)}`);}finally{b.disabled=false;}}
async function importFile(file?:File){if(!file||busy)return;try{if(file.size>8_000_000)throw Error('文件过大，请选择 8 MB 以内的 JSON 备份。');const next=parseBackup(await file.text());const s=calculate(next);modal('恢复这份备份？',`<div class="import-preview">${icon('file-json')}<b>${esc(file.name)}</b><p>${next.trades.length} 笔交易 · ${s.open.length} 只持仓 · ${next.quotes.length} 条报价${next.cash?` · 现金记录 ${next.cash.records.length} 笔${next.cash.opening?'（含期初余额）':''}`:''}</p><strong>持仓成本 ${money(s.cost)}</strong></div>${!next.cash&&(data.cash?.records.length||data.cash?.opening)?`<p class="notice-inline">此备份没有现金记录，恢复后将移除当前${data.cash.opening?'期初余额':''}${data.cash.records.length?`${data.cash.opening?'和 ':''}${data.cash.records.length} 笔现金记录`:''}。</p>`:''}<p class="dialog-copy">确认后，此备份将替换当前的 ${data.trades.length} 笔交易。建议先导出当前账本。恢复前会自动保存当前账本到本机恢复记录。</p><p id="form-error" class="form-error" role="alert"></p><button class="primary full" id="confirm-import">确认替换并恢复</button>`);modalRoot.querySelector<HTMLButtonElement>('#confirm-import')!.onclick=()=>void submit(async()=>{if(blocked){await store.recover(next);blocked=false;data=next;realData=next;}else await commit(next,'导入备份前');});}catch(e){modal('无法导入备份',`<p class="form-error">${esc(e instanceof Error?e.message:String(e))}</p><p class="dialog-copy">当前账本未改变。</p>`);}finally{const i=app.querySelector<HTMLInputElement>('#import');if(i)i.value='';}}
