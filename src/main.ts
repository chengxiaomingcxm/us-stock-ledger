...
function csvImportModal(){
 ...
 modalRoot.querySelectorAll<HTMLInputElement>('[name="csv-kind"]').forEach(r=>r.onchange=()=>{kind=r.value as 'trades'|'cash';mapping=defaultMapping();fallback=undefined;orderConfirmed=false;if(table)renderAll();});
 ...
 function updateConfirm(){
  ...
  const blocked=!!analysis.batchError||(kind==='cash'&&!typeMapped()&&fallback===undefined)||(kind==='trades'&&(analysis as TradeImport).orderAmbiguous&&!orderConfirmed);
 ...
 function renderPreview(){
  ...
  const orderHtml=kind==='trades'&&(analysis as TradeImport).orderAmbiguous?`<label class="csv-order-confirm"><input type="checkbox" id="csv-order-check" ${orderConfirmed?'checked':''}>我已核对同一天内先买后卖的顺序（文件时间按美东时间，无法确定顺序时按文件顺序导入）</label>`:'';
 ...
 fileInput.onchange=()=>{const f=fileInput.files?.[0];if(!f)return;void (async()=>{
  try{if(f.size>2_000_000)throw Error('文件过大，请选择 2 MB 以内的 CSV。');
   ...
   table=parseCsv(text);nameEl.textContent=`${f.name} · ${encoding} · ${table.rows.length} 行数据 · ${table.header.length} 列`;error.textContent='';modalGuard=()=>true;mapping=defaultMapping();fallback=undefined;orderConfirmed=false;renderAll();
 ...
 confirmBtn.onclick=()=>{
  ...
  void submit(async()=>{
   let next:Ledger;
   if(kind==='trades'){const selected=(analysis as TradeImport).rows.filter(r=>boxes.find(b=>b.dataset.importRow===String(r.line))?.checked).map(r=>r.trade);next=applyTradeImport(data,selected,()=>crypto.randomUUID());}
