    ...
    var body: some View {
        List {
            ...
            Section(L10n.tr("行情与报价")) {
                Text(L10n.tr("价格模式可选自动、最近收盘或所选接口最新报价。自动模式在休市时段使用已完成日线，盘中使用所选接口；固定收盘模式不使用 Finnhub Key。历史日线可配置 Tiingo Key，并由 Yahoo、Nasdaq 和手工录入兜底。刷新失败保留旧价。"))
            }
            Section(L10n.tr("收益日历")) {
                Text(L10n.tr("收益日历按每个交易日重放账本：当日收益 = 当日收盘市值 − 上一交易日收盘市值 + 当日卖出净额 − 当日买入含费支出。月份用左右箭头切换，格子里直接显示当日收益金额，点按查看按股票的明细；缺少收盘价的交易日显示“待补”且不计入月度合计。同步仍缺数据时，可在当天详情手工补录并永久保留。"))
            }
