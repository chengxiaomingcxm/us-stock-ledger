...
struct HelpView: View {
    var body: some View {
        List {
            ...
            Section("银行月结单 PDF") {
                Text("选择银行的月结单 PDF 后，应用会读取 PDF 的文字层，把带日期的金额行抽成候选记录，并先推断类型（入金 / 出金 / 分红 / 费用）。日期、金额和类型都需要你核对后才能导入，账单原文会显示在每条下方；与账本中日期、类型、金额相同的行默认不勾选。带密码的 PDF 会提示输入密码。扫描件或截图没有文字层，无法读取，请改用银行导出的 CSV。导入只影响现金账本，不涉及持仓。")
            }
            Section("券商 CSV 导入") {
