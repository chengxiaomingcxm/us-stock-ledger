...
struct HelpView: View {
    var body: some View {
        List {
            ...
            Section("银行月结单 PDF") {
                Text("支持汇丰带文字层的投资服务综合结单 PDF，可多选文件并在本机解锁。预览美元股票买卖、关联费用和派付分红，核对后确认；已识别银行编号重复的记录不重复添加，疑似手工重复默认不选。港币基金、扫描件及银行往来账户流水暂不支持。只有分红到账额时不猜测税费；投资结单里没有的入金、出金和期初现金需要另行填写。")
            }
            Section("券商 CSV 导入") {
