...
- **示例模式（Demo Mode）**：一套完整示例账本，看完可以清空，绝不碰你自己的数据。
...
无需注册、无需数据。打开 **设置 → 载入示例账本** 就能浏览一套示例持仓（买卖、分红、费用、期初余额），准备录自己的数据时用 **退出示例并清空账本**。示例模式是隔离的：绝不会写进你的真实账本。

### 演示视频（规划中）

计划录制一段 60–90 秒的作品展示视频并嵌入这里 —— 启动 → 试示例 → 持仓首页 → 持仓 → 记一笔 → 收益日历 → 导入 → 设置。是展示片，不是教程。
...
- 备份是**你自己**导出的文件（**设置 → 导出账本备份**），存哪由你决定。
