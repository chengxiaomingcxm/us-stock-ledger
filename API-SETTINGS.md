...
进入底部「设置 → 行情 API 设置」，选择来源、测试连接后保存。无需重装。
...
API 配置与交易账本分开保存。密钥位于本机 Preferences，**不是加密密钥保险库**，不会进入账本备份、GitHub 源码或日志。导入旧备份不改变 API 设置；换设备后需重新填写。应用不会发送交易数量、成交金额或备注给行情服务。
