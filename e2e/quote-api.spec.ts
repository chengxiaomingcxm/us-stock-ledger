...
test('API 设置校验、连接测试、重启保存与切回默认来源',async({page})=>{
 ...
 await page.goto('/');await page.getByRole('button',{name:'设置',exact:true}).click();await page.getByRole('button',{name:/行情 API 设置/}).click();
 ...
 await page.reload();await page.getByRole('button',{name:'设置',exact:true}).click();await page.getByRole('button',{name:/行情 API 设置/}).click();
 ...
 await page.getByRole('combobox',{name:'数据源',exact:true}).selectOption('yahoo');await page.getByRole('button',{name:'保存设置',exact:true}).click();await expect(page.getByRole('button',{name:/行情 API 设置/})).toContainText('Yahoo Finance');
});
...
test('手机尺寸下触控、弹窗和页面无横向溢出',async({page})=>{
 ...
 await page.getByRole('button',{name:'设置',exact:true}).click();await page.getByRole('button',{name:/行情 API 设置/}).click();
