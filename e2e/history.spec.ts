...
test('收益日历、历史重算、两种备份导出与本机恢复',async({page})=>{
 ...
 for(const [id,history] of [['export',true],['export-compatible',false]] as const){const event=page.waitForEvent('download');await page.locator('#'+id).click();if(id==='export-compatible')await page.getByRole('button',{name:'继续导出',exact:true}).click();const download=await event;const data=JSON.parse(readFileSync((await download.path())!,'utf8'));expect(!!data.history).toBe(history);expect(data.trades[0].note).toBe('保留备注');expect(data.version).toBe(1);}
