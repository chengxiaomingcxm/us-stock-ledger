...
- 收益页新增现金账本：期初余额、入金、出金、手动分红与预扣税费、账户费用；余额汇总并区分外部资金净流入与投资现金流入，入金出金不直接计为盈亏。
- 新增券商 CSV 导入：交易 / 资金流水两种 CSV，自动识别中英文表头，导入前展示字段映射、逐行预览、错误行与疑似重复记录（默认跳过），确认后写入；导入记录标记 CSV 来源，重复导入不重复记账。
- 现金为可选数据字段：旧备份、本机账本直接读取，无需迁移即可继续使用；期初余额需对照券商账单手动设置。完整备份包含现金记录，通用备份剔除现金与历史。
- 现金记录增删改与 CSV 导入前均先写本机恢复记录；导入校验失败不覆盖可用账本。原有交易成本与已实现收益计算不变。
- 131 项单元测试与 24 项浏览器流程通过；TypeScript 检查与生产构建通过。待云端 iOS 构建、IPA 校验与 iPhone 真机验证。
